...
def glob_matching(fn, fmt):
    ...
    if len(matched_fns) == 0:
        raise Exception(
            "You submitted a {} file, but we didn't find submission.csv in it. Please check your submission.".format(
                fmt))
    if len(matched_fns) > 1:
        raise Exception(
            "You submitted a {} file, but there are more than one files named submission.csv in it. Please check your submission.".format(
                fmt))
...
def read_submission(submit_path, reference, k=5):
    ...
    with open(real_submit_path) as fin:
        for line in fin:
            ...
            if len(records[1:]) != k:
                raise Exception(
                    'Query-id {} has wrong number of predicted product-ids! Require {}, but {} founded.'.format(qid, k,
                                                                                                                len(
                                                                                                                    records[
                                                                                                                    1:])))
            # check whether there exists an empty prediction for any query
            if any([len(r) == 0 for r in records[1:]]):
                raise Exception('Query-id {} has an empty prediction at rank {}! Pleace check again!'.format(qid,
                                                                                                             records[
                                                                                                             1:].index(
                                                                                                                 "") + 1))
            ...
            for rank, r in enumerate(records[1:]):
                if not all([char in string.digits for char in r]):
                    raise Exception(
                        'Query-id {} has an invalid prediction product-id \"{}\" at rank {}'.format(qid, r, rank + 1))
            # check whether there are duplicate predicted products for a single query
            if len(set(records[1:])) != k:
                raise Exception(
                    'Query-id {} has duplicate products in your prediction. Pleace check again!'.format(qid))
    ...
    if len(nopred_qids) != 0:
        raise Exception('The following query-ids have no prediction in your submission, please check again: {}'.format(
            ", ".join(nopred_qids)))
...
standard_path = '../../data/valid/valid_answer.json'
